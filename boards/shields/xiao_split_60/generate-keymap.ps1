...
$keymapPath = Join-Path $PSScriptRoot 'xiao_split_60.keymap'
...
function Flush-Layer {
    ...
    if ($rows.Count -ne 5) {
        throw "Layer '$currentName' has $($rows.Count) row(s), expected 5"
    }
...
foreach ($line in $lines) {
    ...
    if ($tokens.Count -ne 12) {
        throw "Layer '$currentName' row has $($tokens.Count) token(s), expected 12: $trimmed"
    }
    $rows += , $tokens
}
Flush-Layer
